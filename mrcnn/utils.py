...
def compute_overlaps_masks(masks1, masks2):
    ...
    if masks1.shape[-1] == 0 or masks2.shape[-1] == 0: # Instance is shape[-1] not shape[0]
...
def compute_ap(gt_boxes, gt_class_ids, gt_masks,
    ...
    recall_div = len(gt_match)
    if recall_div == 0: # Avoid division by zero
        recall_div = 1
    recalls = np.cumsum(pred_match > -1).astype(np.float32) / recall_div 
